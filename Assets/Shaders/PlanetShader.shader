Shader "Custom/PlanetShader"
{
	...
	result = lerp(z0, z1, u.x) + (z2 - z0) * u.y * (1.0 - u.x) + (z3 - z1) * u.x * u.y; return result;
	}
	void vert(inout appdata_full v)
	{
		float height = noise(v.texcoord, 5) * 0.75 + noise(v.texcoord, 30) * 0.125 + noise(v.texcoord, 50) * 0.125; v.color.r = height + _Height;
	}
	void surf(Input IN, inout SurfaceOutput o)
	{
		...
		if (height < 0.45) {
			...
			color.x = 0.10; color.y = 0.60; color.z = 0.30;
			}
		else {
			color.x = 0.60;
			color.y = 0.30; color.z = 0.30;
			}
		o.Albedo = color.rgb;
